...
def predict(
    ...
    **kwargs,
    ...
    if DOLocationID is not None or PULocationID is not None or trip_distance is not None:
        ...
        inputs = combine_features(inputs)
    print("Model settings keys:", model_settings.keys())

    if 'xgboost_1' not in model_settings:
        raise KeyError("Model settings must include 'xgboost' with both a model and vectorizer.")
    
    model, vectorizer = model_settings['xgboost_1']
